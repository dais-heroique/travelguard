...
import { useTravelLocation } from "@/hooks/use-travel-status";
...
export default function MapScreen() {
  ...
  const { location, isLoading: isLocationLoading } = useTravelLocation();
  ...
  return (
    <ScreenContainer edges={["top", "left", "right"]} containerClassName="bg-background">
      ...
      <FlatList data={places} keyExtractor={(item) => item.id} showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom + 96, 112) }]} ListHeaderComponent={<>
        <View style={[styles.mapCard, { borderColor: colors.border, backgroundColor: colors.surface }]}><NativeRiskMap places={places} userLocation={location ? { latitude: location.latitude, longitude: location.longitude } : undefined} onSelect={setSelectedPlace} /></View>
...
const styles = StyleSheet.create({
  header: { paddingHorizontal: 20, paddingTop: 6, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  ...
  content: { padding: 20, paddingTop: 6, paddingBottom: 32 },
